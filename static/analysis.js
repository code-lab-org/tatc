$(document).ready(function() {
  ...
  function collectCoverage() {
    ...
    $.ajax({
      ...
      data: JSON.stringify({
        ...
        sampleDistance: $('#coverage-override').is(':checked') ? $("#point-distance").val()*1000 : null
      }),
  ...
  function checkProgress(groupId) {
    $.get(
      "/analyze/coverage/"+groupId+"/progress",
      function(progress) {
  ...
  function checkCoverage(taskId) {
    $.get(
      "/analyze/coverage/"+taskId+"/status",
      function(status) {
        if(status.ready) {
          $.get(
            "/analyze/coverage/"+taskId+"/results",
            function(results) {
  ...
  $("#coverage-dialog").on("show.bs.modal", function() {
    ...
    _.forEach($("#coverage-analyze").data("points"), function(feature) {
        features.push({
          ...
          obsCount: feature.properties.num_obs
        });
    });
